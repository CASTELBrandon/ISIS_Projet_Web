...
function buttonArrowRightTrigger() {
  var j = iterator-8;
  for(j; j < iterator && j+8 < nbSlider ; j++) {
  ...
  if (iterator < nbSlider) {
    if (nbSlider-iterator > 8) {
      iterator = iterator + 8;
    } else {
      sliderCounter = nbSlider-iterator;
...
function buttonArrowLeftTrigger() {
  ...
  console.log("iterator avant : " + iterator);
  if (iterator > 9) {
    if (nbSlider-iterator < sliderCounter) {
      iterator = iterator - sliderCounter;
    } else {
      iterator = iterator - 8;
    }
...
******************************************/
  ...
  */
  ...
  document.getElementById("buttonConfig6").addEventListener("click", function() {
    actualConfig = configTrigger(configArray[5], actualConfig);
  });
  document.getElementById("buttonConfigReset").addEventListener("click", function() {
    sendMessage("C1.Rese."+actualConfig.nameConfig,"general");
  });
